...
Map<String, double> getEquationOfTime(double JC) {
  var Sun_Geom_Mean_Long =
      (280.46646 + JC * (36000.76983 + JC * 0.0003032)) % 360; // M0 °

  double Sun_Geom_Mean_Anom = 357.52911 +
      JC * (35999.05029 - 0.0001537 * JC); // Mean Anomaly of the Sun °

  double Eccent_Earth_Orbit =
      0.016708634 - JC * (0.000042037 + 0.0000001267 * JC);

  double Sun_Eq_of_Center =
      (Sin(Sun_Geom_Mean_Anom) * (1.914602 - JC * (0.004817 + 0.000014 * JC)) +
  ...
  double Sun_Apparent_Long =
      Sun_True_Long - 0.00569 - 0.00478 * Sin(125.04 - 1934.136 * JC);

// ε = 23°26′21.45″ − 46.815″ T − 0.0006″ T2 + 0.00181″ T3
  double Mean_Obliquity_Ecliptic = (23 +
      (26 + ((21.448 - JC * (46.815 + JC * (0.00059 - JC * 0.001813)))) / 60) /
          60);

  double Oblique_Correction =
      Mean_Obliquity_Ecliptic + 0.00256 * Cos(125.04 - 1934.136 * JC);

  double Sun_Declination =
      aSin(Sin(Oblique_Correction) * Sin(Sun_Apparent_Long));

  double gamma = Tan(Oblique_Correction / 2) * Tan(Oblique_Correction / 2);

  double Equation_of_Time = 4 *
      Degrees(gamma * Sin(2 * Sun_Geom_Mean_Long) -
          2 * Eccent_Earth_Orbit * Sin(Sun_Geom_Mean_Anom) +
          4 *
              Eccent_Earth_Orbit *
              gamma *
              Sin(Sun_Geom_Mean_Anom) *
              Cos(2 * Sun_Geom_Mean_Long) -
          0.5 * gamma * gamma * Sin(4 * Sun_Geom_Mean_Long) -
          1.25 *
              Eccent_Earth_Orbit *
              Eccent_Earth_Orbit *
              Sin(2 * Sun_Geom_Mean_Anom));
